...
//const localEnv = require('./load-local-env.cjs');
...
function getDatabaseSslConfig() {
  ...
  if (process.env.DB_SSL_CA_PATH) {
    ...
    if (caPath.startsWith(process.cwd())) {
      throw Object.assign(new Error('Refusing to load database CA from the project directory'), {
        status: 500,
        code: 'DB_SSL_CA_PATH_IN_PROJECT'
      });
    }
    return {
...
function getPool() {
  ...
  if (missing.length > 0) {
    throw Object.assign(new Error(`Missing database environment variables: ${missing.join(', ')}`), {
      status: 500,
      code: 'DB_ENV_MISSING'
    });
  }
...
async function ensureSchemas() {
  getPool();
  if (process.env.SKIP_SCHEMA_MIGRATIONS === 'true') return;
  if (!schemaReady) {
...
function shouldExposeDebugError(request) {
  if (process.env.DEBUG_API_ERRORS === 'true') return true;
...
function handleError(error, request = null) {
  ...
  if (status === 500 && request && shouldExposeDebugError(request)) {
    return json({
      error: error.message,
